...
import { Brain, Lightbulb, TrendingUp, Sprout, CloudRain, TestTube, Bug, Droplets, Star } from 'lucide-react';
...
const AIAdvisoryPage: React.FC = () => {
  ...
  const handleSearch = async (query: SearchQuery) => {
    ...
    try {
      ...
      // Found location: fullLocationName (lat, lon)
      ...
      switch (query.queryType) {
        case 'weather':
          try {
            ...
            try {
              ...
              // Forecast data unavailable, using current weather data
        ...
        case 'crop-recommendation':
          ...
          const transformedRecommendations = cropRecommendationData.recommendations.map((rec: CropRecommendation, index: number) => ({
            id: rec.id || index.toString(),
            name: rec.name || 'Unknown Crop',
            suitability: rec.suitability || 70,
            plantingDate: rec.plantingDate || 'Seasonal',
      ...
      case 'pest-control':
        try {
          // Generating pest control advice...
          const currentWeather = await weatherService.getCurrentWeather(lat, lon);
          
          // Use aimlService to get pest control recommendations with real-time data
          // Use a supported crop type to avoid errors
          const supportedCropType = getSupportedCropType(query.cropType || 'General Crops');
          const pestControlData = await aimlService.getPestControl({
            cropType: supportedCropType,
            location: { lat, lon },
      ...
      case 'irrigation':
        try {
          ...
          try {
            ...
            // Forecast unavailable, using current weather for irrigation advice
          }
          ...
          const irrigation = {
            ...
            dailyForecast: irrigationData.weatherContext ? {
              expectedRainfall: irrigationData.weatherContext.forecastedRainfall || 0,
              irrigationNeeded: irrigationData.recommendedWaterAmount || 0,
              recommendation: irrigationData.recommendations?.[0] || 'No specific recommendation'
            } : undefined,
            irrigationSchedule: irrigationData.schedule?.map((item: { date?: string; rainfall?: number; effectiveRainfall?: number; waterAmount?: number; priority?: string }) => ({
              date: item.date || new Date().toISOString(),
              rainExpected: `${item.rainfall || item.effectiveRainfall || 0}mm`,
              irrigationNeeded: ((item.waterAmount as number) || 0) > 0,
              amount: `${item.waterAmount || 0}mm`,
              timing: item.priority || 'Any time'
            })) || [],
            methods: irrigationData.irrigationMethods?.map((method: { name?: string; efficiency?: number | string; suitability?: string; cost?: string }) => ({
              name: method.name || 'Unknown Method',
              efficiency: method.efficiency?.toString() || 'Unknown',
              suitability: method.suitability || 'Unknown',
              cost: method.cost || 'Unknown'
            })) || [],
            tips: irrigationData.conservationTips || []
          };
          
          setResults({ irrigation });
          // Irrigation advice generated
        } catch {
...
// Add helper function to map crop types to supported ones
const getSupportedCropType = (cropType: string): string => {
  const supportedCrops = ['maize', 'rice', 'beans', 'tomato', 'cassava', 'coffee'];
  const cropTypeLower = cropType.toLowerCase();
  
  // Direct match
  if (supportedCrops.includes(cropTypeLower)) {
    return cropTypeLower;
  }
  
  // Map similar crop types
  const cropMap: Record<string, string> = {
    'corn': 'maize',
    'wheat': 'maize', // Use maize as fallback for wheat
    'barley': 'maize',
    'sorghum': 'maize',
    'millet': 'maize',
    'oats': 'maize',
    ...
    'chickpeas': 'beans',
    'soybeans': 'beans',
    'potatoes': 'tomato', // Similar water requirements
    'sweet potatoes': 'cassava',
    'yams': 'cassava',
    'general crops': 'maize'
  };
  
  return cropMap[cropTypeLower] || 'maize'; // Default to maize
};