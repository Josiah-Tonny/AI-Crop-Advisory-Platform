...
// Simple logger
const logger = {
  info: (msg) => console.log(`[IRRIGATION] ${new Date().toISOString()} - ${msg}`),
  error: (msg) => console.error(`[IRRIGATION ERROR] ${new Date().toISOString()} - ${msg}`)
};
...
router.post('/recommend', async (req, res) => {
  try {
    logger.info('Received irrigation recommendation request');
    ...
    logger.info(`Fetching weather for location: ${lat}, ${lon}`);
    ...
    try {
      ...
      logger.info(`Weather data fetched successfully. Forecast days: ${forecast.length}`);
    } catch (weatherError) {
    ...
    if (plantingDate) {
      ...
      logger.info('No planting date provided, assuming 30 days after planting');
    }

    // === CALCULATE ET0 (REFERENCE EVAPOTRANSPIRATION) ===

    logger.info('Calculating reference evapotranspiration (ET0)');

    let et0;
    try {
      et0 = calculateET0(currentWeather, lat, new Date());
      logger.info(`ET0 calculated: ${et0} mm/day`);
    } catch (et0Error) {
      logger.error(`ET0 calculation error: ${et0Error.message}`);
      return res.status(500).json({
        success: false,
    ...
    logger.info(`Getting crop coefficient for ${cropType}, ${daysAfterPlanting} days after planting`);

    let cropData;
    try {
      cropData = getCropCoefficient(cropType, daysAfterPlanting);
      logger.info(`Crop coefficient: ${cropData.kc}, Stage: ${cropData.stage}`);
    } catch (cropError) {
      logger.error(`Crop coefficient error: ${cropError.message}`);
      return res.status(400).json({
        success: false,
    ...
    logger.info(`Calculating soil water balance for ${soilType} soil`);
    ...
    if (currentMoisture !== null && currentMoisture !== undefined) {
      ...
      logger.info(`Estimated soil moisture: ${estimatedMoisture}%`);
    }

    // === GENERATE IRRIGATION SCHEDULE ===

    logger.info('Generating 7-day irrigation schedule');
    ...
    try {
      ...
      logger.info(`Schedule generated with ${schedule.length} days`);
    } catch (scheduleError) {
      logger.error(`Schedule generation error: ${scheduleError.message}`);
      return res.status(500).json({
        success: false,
    ...
    logger.info('Irrigation recommendation generated successfully');
